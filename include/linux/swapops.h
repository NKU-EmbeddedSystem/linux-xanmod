...
#define SWP_ENTRY_ALIVE_VERSION (4)
#define SWP_ALIVE_VERSION_CHECK() BUILD_BUG_ON(SWP_ENTRY_ALIVE_VERSION <= sizeof(long) / sizeof(char))
#define SPEC_MARK_SAFETY_CHECK() BUILD_BUG_ON(SWP_ENTRY_ALIVE_VERSION > SWP_ENTRY_MAX_SPEC)
...
static inline int swp_entry_test_special(swp_entry_t entry){
	return ((entry.val) & SWP_SPECIAL_MASK) >> SWP_SPECIAL_SHIFT;
}

static inline void swp_entry_set_special(swp_entry_t* entry, unsigned long num){
	num = num % ((1 << SWP_SPECIAL_MARK));
	num = (num << SWP_SPECIAL_SHIFT) & SWP_SPECIAL_MASK;
	entry->val = (entry->val & (~SWP_SPECIAL_MASK)) | num;
}
...
static inline int swp_entry_test_ext(swp_entry_t entry){
	return ((entry.val) & SWP_EXT_MASK) >> SWP_EXT_SHIFT;
}

static inline void swp_entry_set_ext(swp_entry_t* entry, unsigned long num){
...
#define swp_entry_set_unready(pentry) swp_entry_set_ext(pentry, swp_entry_test_ext(*pentry)|0x1)
#define swp_entry_set_locked(pentry) swp_entry_set_ext(pentry, swp_entry_test_ext(*pentry)|0x2)
#define swp_entry_test_unready(pentry) (swp_entry_test_ext(*pentry) & 0x1)
#define swp_entry_test_locked(pentry) (swp_entry_test_ext(*pentry) & 0x2)
