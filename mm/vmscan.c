...
collect_fail_lock_keep:
	...
	while (!list_empty(&folio_list)) {
		...
		// VM_BUG_ON_FOLIO(folio_test_dirty(folio), folio); //do_swap_page after folio_free_swap cleanup
...
#endif /*CONFIG_LRU_GEN_STALE_SWP_ENTRY_SAVIOR*/
#endif
...
retry:
	...
	while (!list_empty(folio_list)) {
		...
		if (folio_test_anon(folio) && folio_test_swapbacked(folio)) {
			swap_space_left = -12;
			VM_WARN_ON_FOLIO((!folio_test_swapcache(folio)) && folio_test_stalesaved(folio), folio);
			if (!folio_test_swapcache(folio)) {
		...
		//if stale saved arrived here at a safe state, simply continue next step	
		if (folio_test_stalesaved(folio)){
			if (!folio_test_writeback(folio) && !folio_test_dirty(folio) 
				&& folio_test_swapbacked(folio) && (folio_nr_pages(folio) == 1)){
	...
	list_for_each_entry_safe_reverse(folio, next, &list, lru) {
		if (!folio_evictable(folio)) {
			...
			if (folio_test_stalesaved(folio) || folio_test_swappriohigh(folio))
				pr_info("0 try evict folio[%p] rclm[%d]d[%d]wb[%d]ac[%d]map[%d]lock[%d]", 
