		...
		if (pte_present(ptent)) {
			...
			if (!delay_rmap) {
				...
				if (unlikely(page_mapcount(page) < 0)){
					pr_err("print_bad_pte 1 addr[%lx] page[%p] cnt[%d]", 
 ...
 */
static bool __folio_retry(struct folio *folio, struct mm_struct *mm,
			 unsigned int flags)
{
...
vm_fault_t do_swap_page(struct vm_fault *vmf)
{
	...
	swp_entry_t entry, migentry, orientry;
	pte_t pte;
	int locked;
	vm_fault_t ret = 0;
	void *shadow = NULL;
	...
	if (unlikely(migentry.val)) {
		if (swp_entry_test_locked(&migentry))
		{
			...
			if (unlikely(folio && folio_test_stalesaved(folio))){
				MULTISWAP_MIG_INFO("in process folio[%p]ref[%d], need ref_sub", folio, folio_ref_count(folio));
	...
	if (swapcache){
		...
		if (unlikely(migentry.val)){ //!0
		 	if (non_swap_entry(migentry)){ //this is a BUG; remapping FAILURE
				...
				if (swp_entry_test_unready(&migentry)){ //UNDER MIGRATION
					...
					MULTISWAP_MIG_INFO("success remap [%lx]->[%lx]", orientry.val, migentry.val);
	...
	if (!folio) {
		...
		if (data_race(si->flags & SWP_SYNCHRONOUS_IO) && __swap_count(entry) == 1 && !invalid_remap)
		{
			...
			MULTISWAP_MIG_INFO_ON(unlikely(data_race(si->flags & SWP_SYNCHRONOUS_IO) && migentry.val), 
				"ASYNC_IO si[%d] on sync entry[%lx] cnt[%d] inv[%d]", 
			...
			if (unlikely(migentry.val)) { //use remap
				...
				if (invalid_remap){
					...
					CPU 0 : do_swap_page:
								swap_cache_get_folio -> nothing
				    CPU 1 : enable_reamp_remap(origin->migrate) //will fail because we locked it
					CPU 0 : do_swap_page:
						folio == NULL; invalid entry
				 	*/
				}
				...
				if (page_allocated){
					...
					MULTISWAP_MIG_INFO("__read_sw$_async_save folio[%p] will triger ref drop skip [%d] at cleanup", 
						page, folio_ref_count(page_folio(page)));
				}

				if (page) {
					ClearPageStaleSaved(page);
					MULTISWAP_MIG_INFO("_async_save complete new/$ folio[%p]pri[%lx]ref[%d]  entry[%lx]uptodate[%d]", 
						page_folio(page), page_private(page), folio_ref_count(page_folio(page)),
						entry.val, folio_test_uptodate(page_folio(page)));
				}
			}
			else {
		...
		if (!folio) {
			...
			if (likely(pte_same(*vmf->pte, vmf->orig_pte)))
				ret = VM_FAULT_OOM;
			if (unlikely(migentry.val))
				pr_err("somebody else faulted in this pte entry[%lx] migentry[%lx]", 
	...
	if (swapcache) { //we do the clean
		...
		if (unlikely(folio_test_stalesaved(folio) && folio == swapcache)){ 
			/*
			 * folio locked already, this is still a stale saved page in this case, 
			 * two kind of state in this case this is a before free_unref page, that
			 *  might be in before enabled or also it can be before enabled. 
			 */
			VM_BUG_ON_FOLIO(!(folio_nr_pages(folio) == 1), folio);
			VM_BUG_ON_FOLIO(non_swap_entry(orientry), folio);
			VM_BUG_ON_FOLIO(non_swap_entry(migentry), folio);
			if (valid_remap && !folio_test_writeback(folio) && !folio_test_dirty(folio) 
					&& folio_test_swapbacked(folio) && folio_test_swapcache(folio)) 
			{  
				/*
				 * we don't need to keep the remap any more, because a real pagefault is happing, 
				 * it will then decide weather or not realloc the entry, so we do the clear here.
				 */
				if (migentry.val){
					if (!swp_entry_test_ext(migentry)){	//enabled already
						VM_BUG_ON_FOLIO(!valid_remap, folio);
						...
						/* cached in by origin swap$, remap enabled (while read from origin) */
						// pr_err("enabler do the clean 1, folio[%p] $[%d] entry[%lx]->mig[%lx]", 
						// 	folio, folio_test_swapcache(folio), entry.val, migentry.val);
						pr_err("PF5 1 entry[%lx]cnt[%d]->folio[%p] mig cleared wb[%d]sw$[%d] stale[%d] refcount[%d]", 
							orientry.val, __swap_count(orientry) ,folio, folio_test_writeback(folio), 
							folio_test_swapcache(folio), folio_test_stalesaved(folio), folio_ref_count(folio));	
						BUG();
					}
				}
			}
			else if (invalid_remap && !folio_test_dirty(folio) 
					&& folio_test_swapbacked(folio) && folio_test_swapcache(folio)){
				/*
				 * wb is not set, so that folio will not reach enable_remap we only needs to change 
				 * folio's state to not stalesaved so that the clean ups can be done by enabler
				 */
		...
		if (unlikely(!folio_test_swapcache(folio) ||
			...
			if (!(page_private(page) == orientry.val)){
...
#ifdef CONFIG_LRU_GEN_STALE_SWP_ENTRY_SAVIOR_DEBUG
				pr_info("folio[%p]$[%d]ref[%d] holding bad private[%lx] entry[%lx] orientry[%lx][%d]migentry[%lx] cached[%d]", 
							folio, folio_test_swapcache(folio), folio_ref_count(folio), page_private(page), 
							entry.val, orientry.val, __swap_count(orientry), migentry.val, filemaphit);
				privatebug--;
				if (privatebug <= 0){
					BUG();
				}
	...
	if (unlikely(valid_remap || invalid_remap)){
		if (valid_remap) { //valid remap case
			if (should_try_to_free_swap(folio, vma, vmf->flags, 1)){//valid
				...
				MULTISWAP_MIG_INFO("do_swap skip folio_free_swap valid vmf[%d]flag[%lx]folio[%p]ref[%d] ori[%lx][%d] mig[%lx][%d] ksm[%d]$[%d]wb[%d]", 
			...
			if (should_try_to_free_swap(folio, vma, vmf->flags, 2)){ //invalid
				...
				MULTISWAP_MIG_INFO("do_swap skip folio_free_swap inval vmf[%d] folio[%p]ref[%d]entry[%lx]migen[%lx]ksm[%d]$[%d]wb[%d]", 
					vmf->flags & FAULT_FLAG_WRITE, folio, folio_ref_count(folio), entry.val,  
					migentry.val, folio_test_ksm(folio), folio_test_swapcache(folio), folio_test_writeback(folio));
				folio_set_swappriohigh(folio);
			}
		}
	}
		...
		if (reason == 1){ // normal
			...
			MULTISWAP_MIG_INFO_ON(swp_entry_test_special(entry) > 1 && folio_test_swapcache(folio), 
				"do_swap SKIP folio_free_swap[%lx] vmf[%d] vmaflag[%lx] folio[%p] $[%d] ref[%d] cnt[%d] ksm[%d] write[%d]wb[%d]", 
				entry.val, vmf->flags & FAULT_FLAG_WRITE, vma->vm_flags, folio, folio_test_swapcache(folio),  folio_ref_count(folio),  
				__swp_swapcount(entry), folio_test_ksm(folio), vmf->flags & FAULT_FLAG_WRITE, folio_test_writeback(folio));
		}
	}
...
// #endif
	...
	if (migentry.val && need_unlock){ 
		MULTISWAP_MIG_INFO("retry ckpt0 folio[%p]ref[%d]pri[%lx]uptodate[%d]", 
...
out_page:
	folio_unlock(folio);
	if (migentry.val && need_unlock){ 
		MULTISWAP_MIG_INFO("retry ckpt1 folio[%p]ref[%d]pri[%lx]", 
			folio, folio_ref_count(folio), folio_swap_entry(folio).val);
		folio_set_swappriolow(folio);
	}
out_release:
	if (folio)
		folio_put(folio);
	if (folio && folio != swapcache && swapcache) {
		folio_unlock(swapcache);
		folio_put(swapcache);
	}
	if (si)
