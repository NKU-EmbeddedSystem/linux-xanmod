	...
	if (entry_ext){
		...
		if (old_entry_ext){
			...
			if (memcgid == _memcgid){ //match
				...
				entry_ext->race_state = SE_RACE_STATE_NORMAL;
				entry_ext->eviction_time = min_seq % 0xFFFF; /* Update eviction time */
			}
			else{
				...
				entry_ext->race_state = SE_RACE_STATE_NORMAL; /* Clear race state */
				entry_ext->eviction_time = min_seq % 0xFFFF; /* Update eviction time */
			}
			...
			entry_ext->hist_ts[SE_HIST_PAGE_ID] = get_unique_page_id(memcg);
			entry_ext->hist_ts[SE_HIST_REFAULT_COUNT] = 0;
			entry_ext->hist_ts[SE_HIST_AVG_DISTANCE] = SE_HIST_INITIAL_AVG_DIST * SE_HIST_SCALE_FACTOR;
			entry_ext->race_state = SE_RACE_STATE_NORMAL;
			entry_ext->eviction_time = min_seq % 0xFFFF;
		}
#endif
	}
	else{
			...
			int swap_level, unsigned long entry, bool* abandon_shadow)
{
	...
	if (entry_is_entry_ext(shadow) > 0){
		...
		current_refault_dist = (min_seq % 0xFFFF - entry_ext->eviction_time) % 0xFFFF;
...
skip_count:
	...
	if (shadow && (entry_is_entry_ext(shadow) == 1)) {
		/* Use current min_seq for trace instead of eviction-time min_seq */
		unsigned long refault_token = (min_seq << LRU_REFS_WIDTH) | (token & ((1UL << LRU_REFS_WIDTH) - 1));
		trace_folio_ws_chg_se(folio, va, (unsigned short)memcg_id, refault_token, refs, 1, swap_level, -2, shadow, entry);
	} else
